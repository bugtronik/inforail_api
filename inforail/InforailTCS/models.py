...
class SpErametCodeEquipementGmao(models.Model):
    ordre_tri = models.IntegerField(db_column='ORDRE_TRI', primary_key=True)  # Field name made lowercase.
...
class SpErametRefVehiculesMini(models.Model):
    id_vehicule = models.IntegerField(db_column='ID_VEHICULE', blank=True, null=True, primary_key=False)  # Field name made lowercase.
...
class SpErametVtrainSegmentMini(models.Model):
    ...
    class Meta:
        managed = False
        db_table = '[tcs].[SP_ERAMET_VTRAIN_SEGMENT_MINI]'
...
class SpErametVwagon(models.Model):
    rr_vehiclenumber = models.CharField(db_column='rr_VehicleNumber', max_length=200, blank=True, null=True, primary_key=False)  # Field name made lowercase.
